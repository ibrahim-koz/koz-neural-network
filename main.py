...
from activation_functions import argmax, swish_prime, swish, log_softmax_prime, log_softmax
...
output_number = 6
size_of_hidden_layer = 250

# this line is used to catch the errors arising from numpy.
# np.seterr(all='raise')

neural_network = NeuralNetwork(cross_entropy, cross_entropy_prime)
neural_network.add_layer(FCLayer(input_number, size_of_hidden_layer))
neural_network.add_layer(ActivationLayer(swish, swish_prime))
neural_network.add_layer(FCLayer(size_of_hidden_layer, 50))
neural_network.add_layer(ActivationLayer(swish, swish_prime))
neural_network.add_layer(FCLayer(50, output_number))
neural_network.add_layer(ActivationLayer(log_softmax, log_softmax_prime))
